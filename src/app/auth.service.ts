...
export class AuthService {
  ...
  public login(email: string, password: string): boolean {
    ...
    this.http.post<any>('https://site222301.tw.cs.unibo.it/api/user/login', { email: email, password: password }, httpOptions)
      .subscribe(response => {
  ...
  public signup(nome: string, cognome:string, email: string, pw: string, sesso:string, data:string, animale: string) {
    ...
    this.http.post<any>('https://site222301.tw.cs.unibo.it/api/user/signup', 
